...
def get_stellar_flux_values(wavelength: float, temperature: float, mag: float):
    star_black_body_radiation = (
        ...
        * 1
        / (np.exp((h * c) / (wavelength * kb * temperature)) - 1)
    )
...
def get_galaxy_flux_values(
    ...
    if galaxy_type in galaxy_types and age in age_types:
        filename = (
            f"{age}_{galaxy_type}_type "
            f"{'emission' if has_emission_line else 'no_emission'}.csv"
        )
        file_path = (
        ...
        galaxy_wavelength, galaxy_flux = get_redshifted_spectrum(
            galaxy_wavelength, galaxy_flux, redshift
        )
...
def get_emission_line_values(
    ...
    line_profile = (
        redshifted_line_flux
        * 1
        / (line_signal * np.sqrt(2 * np.pi))
        * np.exp(
            -(
                (wavelength - redshifted_central_wavelength) ** 2
                / (2 * line_signal**2)
            )
        )
    )
...
def get_sky_spectrum(form_data):
    ...
    if (
        parameters["spectrumPlotOptions"]["multiplyWithMirrorAreaAndEfficiency"]
        == "true"
    ):
        filename = (
            FILES_BASE_DIR
...
def get_sources_spectrum(form_data):
    ...
    for source in sources:
        if source["spectrumType"] == "Blackbody":
            star_flux = get_stellar_flux_values(
                data["wavelength"],
            ...
            galaxy_flux = get_galaxy_flux_values(
                data["wavelength"],
                source["type"],
                source["age"],
                has_emission_line,
                float(source["magnitude"]),
                float(source["redshift"]),
            )
            data["sources_flux"] = data["sources_flux"] + galaxy_flux
        elif source["spectrumType"] == "Emission Line":
            emission_line_flux = get_emission_line_values(
                data["wavelength"],
                float(source["flux"]),
                float(source["centralWavelength"]),
                float(source["fwhm"]),
                float(source["redshift"]),
            )
    ...
    if parameters["spectrumPlotOptions"]["multiplyWithMirrorAreaAndEfficiency"]:
        ...
        data["sources_flux"] = data["sources_flux"] * int(
            parameters["earth"]["mirrorArea"]
        )
